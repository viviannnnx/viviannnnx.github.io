...
imname = './1/CS180_fa2026_proj1_data/emir.tif'
...
def gradient_align(base, moving, window=15):

    base_gradient = gradient_magnitude(base)
    moving_gradient = gradient_magnitude(moving)
...
def pyramid_gradient_align(base, moving, window=5, min_size=64):

    if min(base.shape[:2]) <= min_size:

        return gradient_align(base, moving, window=15)

    small_base = resize_half(base)
    small_moving = resize_half(moving)

    dx_coarse, dy_coarse = pyramid_gradient_align(small_base, small_moving, window=window, min_size=min_size)

    dx_guess = dx_coarse * 2
    dy_guess = dy_coarse * 2

    pre_shifted = shift_im(moving, dx_guess, dy_guess)

    dx_refine, dy_refine = gradient_align(base, pre_shifted, window=window)
...
gb_dx, gb_dy = pyramid_gradient_align(b,g)
rb_dx, rb_dy = pyramid_gradient_align(b,r)
